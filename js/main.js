...
let wordsList = (str, subStr) => {
    ...
    // https://overcoder.net/q/825196/отображение-уникальных-символов-в-строке-только-один-раз

    let func = str.
    toLowerCase().
    replace(/(\.|,|!|\?|;|:|")/gi, '').
    split(' ').
    filter(a => a.indexOf(subStr) > -1);
...
let getLocalDate = (date, isSeconds, isISO) => {
    // let exp = new RegExp(/(?::\d{2}$)/, 'g');
    let finalDate;

    if (!isISO) finalDate = isSeconds ? date.toLocaleString() : date.toLocaleString().slice(0, -3); /*.replace(exp, '');*/
    else {
        ...
        let format1 = years + '-' + months + '-' + days + ', ' + hours + ':' + minutes;
        let format2 = years + '-' + months + '-' + days + ', ' + hours + ':' + minutes + ':' + seconds;

        return finalDate = isSeconds ? format2 : format1;
...
// https://gist.github.com/ihorkatkov/6cdbd49f5901fb70be9a

let getDateAgo = (date, days) => {
    let currentDate = new Date(date);
    currentDate.setDate(currentDate.getDate() - days);
    return currentDate.toLocaleString().slice(0, -10); /*.replace(/,\s+(\d*:\d*:\d*)/gui, '')*/
};
...
let car = new Car(2000, 'Lacetti', 'Chevrolet', 2010);
let car2 = new Car(5000, 'FX50 AWD', 'Infinite', 2020);
